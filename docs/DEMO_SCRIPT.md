...
- **Phone B** (responder) — open `/responder`, enter the responder passcode (set on the deployment), select unit **E-02 Medic Team 2**. Leave it on standby.
- **Laptop** (projector) — open `/command`, log in with the operator ID and password set on the deployment. Leave it on the (empty) dashboard.
